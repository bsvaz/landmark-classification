...
def get_model_transfer_learning(model_name="resnet152", fine_tuning=False, n_classes=50):
    ...
    # Unfreeze last backbone layers if funi_tuning=True
    if fine_tuning:
        params_layer4 = model_transfer.layer4.parameters()
        for param in params_layer4:
    ...
    #    outputs
    model_transfer.fc  = nn.Sequential(nn.Linear(num_ftrs, 4096),
                                       nn.Dropout(0.2),
                                       ...
                                       nn.Linear(2048, n_classes))# YOUR CODE HERE
